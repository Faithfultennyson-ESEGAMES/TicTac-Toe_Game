...
class AudioManager {
  ...
  setMuted(muted) {
    ...
    // Always mirror mute to HTMLAudio fallbacks as well. Some WebViews expose
    // AudioContext successfully but fail decoding one or more files; those
    // sounds then use <audio> even though WebAudio exists. Muting only the
    // master gain would leave those fallback streams audible.
    Object.values(this.elements).forEach((el) => {
      try { el.muted = this.muted; } catch (error) {}
    });

    if (!this.audioContext) {
      // Pure <audio> fallback path: pause loops while muted and restart the
      // wanted background music after unmuting.
      if (this.muted) {
        this.stopMusic();
      ...
      // WebAudio exists but its music buffer/source may have failed, in which
      // case startMusic() will safely use the HTMLAudio fallback.
      this.startMusic();
    }
  ...
  startMusic() {
    ...
    // Primary: looping buffer source through the music gain. If the HTMLAudio
    // fallback started while the buffer was still decoding, stop that fallback
    // before switching to WebAudio so one client can never play two BG tracks.
    if (this.audioContext && this.buffers.bgMusic) {
      if (this.musicSource) return; // already playing through WebAudio
      if (this.musicElement && !this.musicElement.paused) {
        try { this.musicElement.pause(); this.musicElement.currentTime = 0; } catch (error) {}
      }
      this.resume();
      try {
        const source = this.audioContext.createBufferSource();
    ...
    // Fallback: <audio> element. Never start it while a WebAudio music source
    // exists, and don't replay an element that is already running.
