...
// Creates the initial log object for a new session and logs the 'session.created' event.
function startSessionLog(session) {
  ...
  appendEvent(session.sessionId, 'session.created', { turn_duration_sec: session.turnDurationSec });
}
...
function finalizeLog(session, { winState, winnerPlayerId }) {
