...
class GameClient {
  constructor() {
    ...
    try {
      this.apiBase = this.params.joinUrl ? `${new URL(this.params.joinUrl).origin}/api` : '';
    } catch (e) {
      this.apiBase = '';
    }
  ...
  async init() {
    ...
    try {
      ...
      await this.socketManager.emitWithAck('join', joinPayload);
  ...
  async attemptRejoin() {
    ...
    try {
      await this.socketManager.connect();
      await this.socketManager.emitWithAck('join', {
