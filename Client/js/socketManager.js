...
class SocketManager {
      ...
      try {
        this.socket.emit(event, payload, (response) => {
          ...
          if (response?.success === false) {
            reject(new Error(response.message || `${event} was rejected`));
            return;
          }
  ...
  sendReliably(event, payload, { intervalMs = 3000, maxAttempts = 8, onExhausted } = {}) {
    const key = `${event}:${payload?.sessionId || ''}:${payload?.playerId || ''}`;
    ...
    const fire = () => {
      if (attempts >= maxAttempts) {
        ...
        this.stopReliable(key);
        if (typeof onExhausted === 'function') onExhausted(action?.attempts || attempts);
        return;
      }
      attempts += 1;
      const action = this.reliableActions.get(key);
